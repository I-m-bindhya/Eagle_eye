...
class OCR:
    ...
    def set_language(self, language):
        """
        Sets the self.language parameterself.exchange
    ...
    def ocr(self):
        ...
        while not self.stopped:
            if self.exchange is not None:  # Defends against an undefined VideoStream reference
                # if self.exchange.frame : frame = self.exchange.frame 
                # else : 
                frame = self.exchange.read()[1]
...
def views(mode: int, confidence: int):
    ...
    if mode == 1:
        conf_thresh = 10  # Only shows boxes with confidence greater than 75
        color = (0, 255, 0)  # Green
...
def put_ocr_boxes(boxes, frame, height, crop_width=0, crop_height=0, view_mode=1):
    ...
    if boxes is not None:  # Defends against empty data from tesseract image_to_data
        for i, box in enumerate(boxes.splitlines()):  # Next three lines turn data into a list
            ...
            if i != 0:
                if len(box) == 12:
                    ...
                    print('ocr boxes', conf, conf_thresh)
        ...
        if text.isascii():  # CV2 is only able to display ascii chars at the moment
            cv2.putText(frame, text, (5, height - 5), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 0, 0))
...
def put_crop_box(frame: numpy.ndarray, width: int, height: int, crop_width: int, crop_height: int):
    ...
    cv2.rectangle(frame, (crop_width, crop_height), (width + crop_width, height + crop_height), (144, 238, 144), thickness=1)
    return frame
...
def put_rate(frame: numpy.ndarray, rate: float) -> numpy.ndarray:
    ...
    #cv2.putText(frame, "{} Iterations/Second".format(int(rate)), (10, 35), cv2.FONT_HERSHEY_DUPLEX, 1.0, (255, 255, 255))
    return frame
...
def put_language(frame: numpy.ndarray, language_string: str) -> numpy.ndarray:
    ...
    """
    #cv2.putText(frame, language_string,(10, 65), cv2.FONT_HERSHEY_DUPLEX, 1.0, (255, 255, 255))
    return frame
...
def ocr_stream(crop: list[int, int], source: int = 0, view_mode: int = 1, language=None, img_wi: int = 0, img_hi: int = 0):
    ...
    # print(video_stream.get_video_dimensions())
    ...
    ocr.set_exchange(video_stream.stream)
    ...
    while True:
        ...
        cv2.imshow("Eagle Eye - OCR", frame)
