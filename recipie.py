...
def main():
    ...
    Label_Heading = tk.Label(window, text="RECIPIE MANAGEMENT", font=(22))
    ...
    Frame_Data = tk.LabelFrame(window, text="RECIPE LIST", font=(12), bd=2, relief=tk.GROOVE)
    ...
    def GET_DATA():
        cur.execute('SELECT r.id, r.recipe_alt_name, r.save_selection, u.username, COUNT(roi.id), r.ip_address, r.station_name, r.recipe_date FROM recipes as r LEFT JOIN Users as u ON r.user_id=u.id LEFT JOIN roi ON r.id = roi.receipe_id GROUP BY r.id')
        rows=cur.fetchall()
    ...
    def GET_DATA_SEARCH():
        ...
        cur.execute("SELECT r.id, r.recipe_alt_name, r.save_selection, u.username, r.ip_address, r.station_name, r.recipe_date FROM recipes as r LEFT JOIN Users as u ON r.user_id=u.id where r.recipe_alt_name='" + searchValue + "'")
    ...
    Search_Box['values'] = ("ID","RECIPE NAME","AUTO SAVE", "CREATED BY", "ROI COUNT", "IP ADDRESS", "STATION NAME", "CREATED DATE")
    ...
    Recipes_table = ttk.Treeview(Frame_Database, columns=("ID", "RECIPE NAME","AUTO SAVE", "CREATED BY", "ROI COUNT", "IP ADDRESS", "STATION NAME", "CREATED DATE"), yscrollcommand= Scroll_Y.set,xscrollcommand= Scroll_X.set)
    ...
    Recipes_table.heading("CREATED BY", text="CREATED BY")  #username - created by
    Recipes_table.heading("ROI COUNT", text="ROI COUNT")
    ...
    Recipes_table.column("CREATED BY")
    Recipes_table.column("ROI COUNT")
